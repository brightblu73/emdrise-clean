...
- **Complete User Flow Optimization (Aug 20, 2025)**: Implemented smooth registration → Stripe → homepage → EMDR journey flow. Enhanced auth page with reordered buttons (Create Account first, Sign In second with separator text). Created auth-callback page that automatically redirects verified users to Stripe Checkout. Updated Stripe success URL to redirect to homepage with trial confirmation message. For authenticated users, homepage now shows single "Choose Therapist & Continue Your Journey" button that goes directly to EMDR session without triggering additional Stripe flows.
