...
- **Complete User Flow Optimization & Bug Fixes (Aug 21, 2025)**: Implemented smooth registration → Stripe → homepage → EMDR journey flow. Enhanced auth page with reordered buttons (Create Account first, Sign In second with separator text). Created auth-callback page that automatically redirects verified users to Stripe Checkout. Fixed Stripe success URL redirect issues and webhook user_id association problems. Added comprehensive subscription status checking and fallback user lookup by Stripe customer ID. For authenticated users, homepage now shows single "Choose Therapist & Continue Your Journey" button that goes directly to EMDR session without triggering additional Stripe flows.
