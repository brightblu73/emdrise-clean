...
- **Apple IAP Migration Implementation (Aug 21, 2025) - IN PROGRESS**: Complete migration to Apple In-App Purchases using RevenueCat + Expo EAS Build architecture. Implemented RevenueCat SDK integration with subscription management UI, created Apple IAP-compliant subscription flow, added RevenueCat webhook handlers for backend synchronization, and configured EAS Build for App Store deployment. Mobile app ready for Apple Developer setup and RevenueCat configuration. This replaces Stripe payments with App Store-compliant IAP system.
- **UI Stability & Login Flow Fixes (Aug 21, 2025) - PRODUCTION READY**: Completely eliminated UI flashing issues for authenticated users by removing duplicate authentication state management. Simplified homepage logic to show "Choose Therapist & Continue Your Journey" button for ALL authenticated users, removing confusing trial prompts for logged-in users. Updated auth-callback to always redirect to homepage instead of triggering unnecessary Stripe checkout flows for returning users. Enhanced loading states and visual stability across the application. **Clean, professional user experience confirmed for both new registrations and returning user logins.**
- **Complete User Flow Optimization & Bug Fixes (Aug 21, 2025) - FULLY OPERATIONAL**: Successfully implemented and tested complete registration → Stripe → homepage → EMDR journey flow. Enhanced auth page with reordered buttons (Create Account first, Sign In second with separator text). Created auth-callback page that automatically redirects verified users to Stripe Checkout. Fixed critical Stripe redirect issues by updating success_url and cancel_url to point directly to root route `/` instead of localhost or non-existent routes. Resolved webhook user_id association problems with comprehensive fallback logic (metadata → customer_id → email lookup). Added robust subscription status checking and enhanced logging throughout payment flow. For authenticated users, homepage now shows single "Choose Therapist & Continue Your Journey" button that goes directly to EMDR session without triggering additional Stripe flows. **User flow confirmed working end-to-end.**
- **Stripe Checkout Integration (Aug 20, 2025)**: Migrated from Stripe Payment Elements to hosted Stripe Checkout sessions for improved user experience. Implemented `/api/create-checkout-session` endpoint that creates checkout sessions with 7-day free trials and automatic charging after trial ends. Added webhook handling for subscription events and success page routing. Users now get redirected to Stripe's secure checkout page for payment setup.
- **Duplicate Customer Prevention (Aug 19, 2025)**: Enhanced subscription endpoints to prevent duplicate Stripe customer and subscription creation. Added comprehensive logic to check for existing customers by Stripe ID and email, retrieve existing active subscriptions, and reuse them instead of creating duplicates. Improved logging for better debugging of subscription flow issues.
- **Frontend Subscription Flow Integration (Aug 19, 2025)**: Connected all frontend CTA buttons to Stripe subscription backend with proper authentication checks, loading states, error handling, and user flow routing. Updated `handleSubscriptionFlow` and `handleStartTrial` functions to redirect to Stripe Checkout instead of inline payment forms.
