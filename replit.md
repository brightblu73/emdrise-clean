...
- **Complete User Flow Optimization & Bug Fixes (Aug 21, 2025)**: Implemented smooth registration → Stripe → homepage → EMDR journey flow. Enhanced auth page with reordered buttons (Create Account first, Sign In second with separator text). Created auth-callback page that automatically redirects verified users to Stripe Checkout. Fixed critical Stripe redirect issues by updating success_url and cancel_url to point directly to `/home` route instead of localhost or non-existent `/success` endpoints. Resolved webhook user_id association problems with comprehensive fallback logic (metadata → customer_id → email lookup). Added robust subscription status checking and enhanced logging throughout payment flow. For authenticated users, homepage now shows single "Choose Therapist & Continue Your Journey" button that goes directly to EMDR session without triggering additional Stripe flows.
